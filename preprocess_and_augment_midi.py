...
def augment_midi(midi_path, pitch, tempo):
    ...
    filepath, file_name_with_extension = os.path.split(midi_path)
...
def prep_custom_midi_aug(custom_midi_root, output_dir, valid_p=0.1, test_p=0.2):
    """
    ----------
    Author: Corentin Nelias, Deborah Guimarães
    ----------
    Pre-processes custom midi files that are not part of the maestro dataset, putting processed midi data (train, eval,
    test) into the given output folder while augmenting the training data.
    ----------
    """
    train_dir = os.path.join(output_dir, "train")
    os.makedirs(train_dir, exist_ok=True)
    ...
    for piece in os.listdir(custom_midi_root):
        ...
        midi_path   = os.path.join(custom_midi_root, piece)
...
def parse_args():
    ...
    parser.add_argument("-output_dir", type=str, default="./dataset/maestro", help="Output folder to put the preprocessed midi into.")
...
def main():
    ...
    is_custom_dataset   = args.custom_dataset

    print("Preprocessing midi files and saving to", output_dir)
    if is_custom_dataset:
        prep_custom_midi_aug(root, output_dir)
    else:
